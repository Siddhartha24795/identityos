...
likeness of the applicant — see below), and v4.3 (the first real
third-party browser test ever run — field detection only, nothing
filled or submitted — which found and fixed two real bugs: a blocked
page silently reading as an empty form, and a video-narration script
reading `--` aloud as "hyphen hyphen" — see below) of that brief — see
...
Run the smoke test suite: `make test` (88 tests, ~13s including several real
...
synthetic stand-in would defeat rather than satisfy. A real rendered
example — produced end-to-end by this exact pipeline, not staged —
is committed at
[docs/media/video_statement_draft_v4_2.mp4](docs/media/video_statement_draft_v4_2.mp4)
(3:31, disclosure banner burned into every slide). This is a separate
artifact from the main solution video above: the solution video was
authored during development using standalone tooling, before this module
existed as a product feature — this one is the actual product capability,
generated by calling `services/video_engine` directly. Full story:
[docs/architecture.md](docs/architecture.md)'s v4.2 addendum.

**v4.3 — first real third-party browser test, two real bugs found:**
asked directly whether the browser agent's field detection works against
a real, live site — not just the local synthetic form every prior version
used — `observe()` was pointed at a real HackerEarth page for the first
time (read-only field detection only; nothing was filled or submitted).
It found the target blocked with an HTTP 403 (confirmed independently via
plain `curl` too, not headless-detection specifically), which `observe()`
silently reported as "0 fields, 0 errors" — indistinguishable from "this
page has no form." **Fixed**: the agent now flags any failed HTTP status
or blocked-page title phrasing and halts, the same treatment as a CAPTCHA
widget. Separately, listening to the solution video surfaced a second bug:
its narration script used `--` as an em-dash substitute, which the TTS
engine read aloud as "hyphen hyphen." **Fixed**: narration text now strips
that punctuation before synthesis; the solution video was re-rendered
(4:29 -> 4:27). Full story: [docs/architecture.md](docs/architecture.md)'s
v4.3 addendum.
...
"improvement" verdict before being caught and fixed, (v4.2) an ffmpeg
concat-path bug that broke the very first narrated-draft render attempt,
and (v4.3) a blocked real-world page silently read as an empty form, plus
a narration script reading `--` aloud as "hyphen hyphen" (documented, not
...
[docs/media/solution_video.mp4](docs/media/solution_video.mp4) (4:27).
