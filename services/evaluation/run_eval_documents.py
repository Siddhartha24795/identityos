"""v2.5 — generates one cover letter with each of baseline_plain,
baseline_rag, and identityos_v2_5 (hybrid retrieval + narrative-state
tracking), scores every one with the same verifier as v1/v2, and writes:
  - data/evaluation/results/<tag>/documents/<system>.md   (the actual letter)
  - data/evaluation/results/<tag>/document_summary.json    (scored comparison)
  - data/evaluation/results/<tag>/trajectories/cover_letter__<system>_<section>.{md,json}
"""
...
from services.document_engine.generate import (
    generate_cover_letter_baseline_plain,
    generate_cover_letter_baseline_rag,
...
def run(
    ...
    doc_plain, traj_plain = generate_cover_letter_baseline_plain(provider)
    doc_rag, traj_rag = generate_cover_letter_baseline_rag(ds, provider)
    doc_sys, traj_sys = generate_cover_letter_identityos(ds, embedding_index, provider)

    documents = {"baseline_plain": doc_plain, "baseline_rag": doc_rag, "identityos_v2_5": doc_sys}
    trajectories = {"baseline_plain": traj_plain, "baseline_rag": traj_rag, "identityos_v2_5": traj_sys}

    for name, traj in trajectories.items():
        stem = f"cover_letter__{name}"
    ...
    for name, doc in documents.items():
        (docs_dir / f"{name}.md").write_text(doc.full_text, encoding="utf-8")

    (run_dir / "document_answers.json").write_text(
    ...
    (run_dir / "document_summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
    return summary
