...
**v4.2 addition, prompted directly by the project owner**: pointed out
that this project's own solution video used synthesized narration, not
his real voice, and that many real research/application programs require
a video introduction or pitch. Built `services/video_engine/` — the same
citation-verified generation pipeline as v2.5, applied to a video-statement
script, plus an optional narrated-draft render — with an explicit,
permanent scope boundary decided *before* writing any code: no synthetic
likeness (voice or face) of the applicant, ever. See
docs/architecture.md's v4.2 addendum for the full reasoning.
